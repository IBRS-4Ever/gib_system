...
local CVAR_GibSystem_cam_freemove	= CreateClientConVar("GibSystem_freecam", "0", 0, 1)				--（0/1）是否启用第一人称死亡视角自由移动
...
hook.Add("CalcView", "GibSystem_FirstDeathCam", function(ply,  pos_ply,  ang_ply,  fov)
	...
	if CVAR_GibSystem_cam_enable:GetBool() then
		...
		if Gib:LookupAttachment('eyes') > 0 and Gib.ang_smooth_last then
			local attach_data = Gib:GetAttachment(Gib:LookupAttachment('eyes'))
			local pos = attach_data.Pos
			local ang = attach_data.Ang

			if CVAR_GibSystem_cam_freemove:GetBool() then
				ang = LerpAngle(0.5, ang, ang_ply)
				ang = Angle(ang_ply.p, ang_ply.y, ang.r)
				--ang_ply:RotateAroundAxis(ang:Up(),90)
				--ang_ply:RotateAroundAxis(ang:Right(),90)
				...
				pos.z = math.max(pos.z, rd.HitPos.z + 5)
			end
